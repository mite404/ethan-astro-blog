...
queries at 950/760/640/570/480px; Tailwind `md:`/`lg:` in `index.astro`). They fire off _viewport_
...
whole 390→1280px range; media queries only change _flow_.
...
1. **Continuous scaling** → `clamp()` tokens, active across the _entire_ 390→1280px range, so
   scaling stays smooth _through_ both breakpoints.
2. **Flow changes** → two media queries that only change _arrangement_, never size.
